...
class PctChangeFunction(PandasApiAppliedFunction):
    ...
    def validate(self) -> bool:
        ...
        if self.__periods not in valid_periods:
            raise NotImplementedError(
                f"The 'periods' argument of the pct_change function is only supported for values {valid_periods},"
                f" but got: periods={self.__periods!r}"
            )

        if self.__freq is not None:
            raise NotImplementedError(
                f"The 'freq' argument of the pct_change function is not supported, but got: freq={self.__freq!r}"
            )

        if self.__axis not in [0, "index"]:
            raise NotImplementedError(
                f"The 'axis' argument of the pct_change function must be 0 or 'index', but got: axis={self.__axis!r}"
            )
